window.addEventListener('load', function () {
	...
	fetch(
		'https://api.themoviedb.org/3/tv/' +
			id +
			'?api_key=c0e01d0df95b98b689dcb3af16007742&language=en-US'
	)
		...
		.then(function (datos) {
			...
			info_conteiner.innerHTML +=
				'<h2><strong>' + datos.name + '</strong></h2>';

			var gnre = datos.genres;
			var losgeneros = '';
			for (var i = 0; i < gnre.length; i++) {
				losgeneros +=
					' <a href="/series_xgenero?id=' +
					datos.genres[i].id +
					'">' +
					datos.genres[i].name +
					'</a> ';
			...
			info_conteiner.innerHTML +=
				'<h3><strong>Géneros: </strong>' + losgeneros + '</h3>';

			info_conteiner.innerHTML +=
				'<h3><strong>Lenguaje original: </strong>' +
				datos.original_language +
				'</h3>';

			info_conteiner.innerHTML +=
				'<h3><strong>Sinopsis:</strong>' +
				' <p><em>' +
				datos.overview +
				'</em></p></h3>';
			info_conteiner.innerHTML +=
			...
			// fecha de estreno
			info_conteiner.innerHTML +=
				'<h3><strong>Fecha de estreno: </strong>' +
				datos.first_air_date +
				'</h3>';

			// poster
			foto_conteiner.innerHTML =
	...
	fetch(
		'https://api.themoviedb.org/3/tv/' +
			id +
			'/videos?api_key=c0e01d0df95b98b689dcb3af16007742&language=en-US'
	)
		.then(function (respuesta) {
			return respuesta.json();
		})
	...
	fetch(
		'https://api.themoviedb.org/3/tv/' +
			id +
			'/recommendations?api_key=c0e01d0df95b98b689dcb3af16007742&language=en-US&page=1'
	)
		.then(function (respuesta) {
			return respuesta.json();
		})
		.then(function (datos) {
			...
			for (var i = 1; i < pel.length; i++) {
				...
				recomen.innerHTML +=
					'<li><a id="click_pelis" href="/info_serie?id=' +
					datos.results[i].id +
					'"> ' +
	...
	document.querySelector('.botonFavorito').onclick = function () {
		...
		if (seriesFavoritos.includes(id)) {
			...
			document.querySelector('.botonFavorito').innerHTML =
				'Agregar a favoritos';
		} else {
			//Lo agrego
	...
	if (seriesFavoritos.includes(idTarde)) {
		document.querySelector('.botonFavoritoTarde').innerHTML =
